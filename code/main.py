...
import os
import sys
from tkinter import N, W, E, S, Text, VERTICAL, END, Tk
...
def cli():
    """
    a simple CLI for program, not use
    """
    txt = input("输入文本: ")
    graph = WordGraph(txt)
    while True:
...
class WordGraphApp:
    """
    the GUI for word graph program
    """
    def __init__(self, app_root):
        ...
        self.create_widgets()
        # 添加选择文件的功能
        file_path = filedialog.askopenfilename(initialdir=os.getcwd(), title="Select Text File",
    ...
    def create_widgets(self):
        ...
        ttk.Button(button_frame, text="Show Directed Graph", command=self.show_directed_graph)\
            .grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(button_frame, text="Query Bridge Words", command=self.query_bridge_words)\
            .grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(button_frame, text="Generate New Text", command=self.generate_new_text)\
            .grid(row=1, column=0, padx=5, pady=5)
        ttk.Button(button_frame, text="Calculate Shortest Path", command=self.calc_shortest_path)\
            .grid(row=1, column=1, padx=5, pady=5)
        ttk.Button(button_frame, text="Random Walk", command=self.random_walk)\
            .grid(row=2, column=0, padx=5, pady=5)
        ttk.Button(button_frame, text="Quit", command=self.quit)\
    ...
    def show_directed_graph(self):
        """
        method for butten Show Directed Graph
        """
    ...
    def query_bridge_words(self):
        """
        method for butten Query Bridge Words
        """
    ...
    def generate_new_text(self):
        """
        method for butten Generate New Text
        """
    ...
    def calc_shortest_path(self):
        """
        method for butten Calculate Shortest Path
        """
        w1 = self.prompt_user("word1")
        w2 = self.prompt_user("word2 (Enter # to calculate \
                              the shortest path from word1 to all words)")
        if w2 != '#':
            ...
            # 展示所有最短路径图片
            self.image_paths = [
                os.path.join('output', f'shortest_path_{i}.jpg') for i in range(num)
            ]
            self.image_index = 0
    ...
    def prompt_user(self, prompt):
        """
        add prompt to input box
        """
        return simpledialog.askstring("Input", prompt)

    def display_output(self, text):
        """
        show output in the text window
        """
    ...
    def create_navigation_buttons(self):
        """
        add navigation buttens under picture
        """
        # 添加导航按钮
        nav_frame = ttk.Frame(self.image_frame, padding="10 10 10 10")
        nav_frame.grid(row=1, column=0, sticky=(E, W))

        ttk.Button(nav_frame, text="Previous", command=self.show_previous_image)\
            .grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(nav_frame, text="Next", command=self.show_next_image)\
            .grid(row=0, column=1, padx=5, pady=5)

    def show_next_image(self):
        """
        method for butten Next
        """
